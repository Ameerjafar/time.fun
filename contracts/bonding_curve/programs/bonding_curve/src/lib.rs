...
pub mod bonding_curve {
    ...
    pub fn buy(ctx: Context<Buy>, amount_out_token: u64) -> Result<()> {
        ...
        // Store mint keys before mutable borrow
        let token_x_mint = state.token_x_mint;
        let token_y_mint = state.token_y_mint;
...
pub struct Buy<'info> {
    #[account(
        mut,
        seeds = [b"pool", pool.token_x_mint.as_ref(), pool.token_y_mint.as_ref()],
        bump
    )]
...
pub struct InitializePool<'info> {
    ...
    // User's token accounts (source of initial liquidity)
    #[account(mut)]
    pub user_token_x_account: Account<'info, TokenAccount>,
    #[account(mut)]
    pub user_token_y_account: Account<'info, TokenAccount>,

    // Pool's token accounts (destination)
    #[account(mut)]
